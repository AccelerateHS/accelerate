...
module Data.Array.Accelerate.AST (
  ...
  PreOpenSeq(..), PreOpenNaturalSeq, PreOpenChunkedSeq, OpenNaturalSeq, OpenChunkedSeq,
...
data Producer index acc aenv a where
  ...
  ProduceAccum :: (Arrays a, Arrays b, Arrays index)
...
data Consumer index acc aenv a where
  ...
  -- lifted. Removed by fusion.
  Iterate        :: (Arrays a, Arrays index)
                 => Maybe (PreExp acc aenv Int)
                 -> PreOpenAfun acc aenv (index -> a -> a)
                 -> acc aenv a
                 -> Consumer index acc aenv a

  -- Simply yield the result of the last Producer when the limit is reached.
  --
  -- Generated by fusion.
  Conclude       :: Arrays a
...
--
type PreOpenChunkedSeq = PreOpenSeq (Scalar (Int, Int))

type NaturalProducer = Producer (Scalar Int)
type ChunkedProducer = Producer (Scalar (Int, Int))
type NaturalConsumer = Consumer (Scalar Int)
type ChunkedConsumer = Consumer (Scalar (Int, Int))
...
--
type Seq index  = PreOpenSeq index OpenAcc ()
