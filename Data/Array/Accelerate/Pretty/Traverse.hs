...
travAcc f c l (OpenAcc openAcc) = travAcc' openAcc
  where
    ...
    travAcc' (Awhile cond body acc)                = combine "Awhile" [travExp f c l cond, travAfun f c l body, travAcc f c l acc]
...
travExp f c l expr = travExp' expr
  where
    ...
    travExp' (While cond body x)        = combine "While" [ travFun f c l (Lam (Body cond)), travFun f c l (Lam (Body body)), travExp f c l x ]
