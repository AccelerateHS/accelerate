    ...
    *bi = thid + blockDim.x;
    ...
    if (IS_BACKWARD)
    {
        ...
        if (IS_FULLBLOCK || i + 3 < numElements)
        {
            ...
            threadScan0[2] = apply(tempData.z, threadScan0[3]);
            threadScan0[1] = apply(tempData.y, threadScan0[2]);
            threadScan0[0] = s_out[*ai]
                           = apply(tempData.x, threadScan0[1]);
        }
        else
        {
            threadScan0[3] = identity();
            threadScan0[2] = apply(((i+2) < numElements) ? d_in[i+2] : identity(), threadScan0[3]);
            threadScan0[1] = apply(((i+1) < numElements) ? d_in[i+1] : identity(), threadScan0[2]);
            threadScan0[0] = s_out[*ai]
                           = apply((i     < numElements) ? d_in[i]   : identity(), threadScan0[1]);
        }
        ...
        if (IS_FULLBLOCK || i + 3 < numElements)
        {
            ...
            threadScan1[2] = apply(tempData.z, threadScan1[3]);
            threadScan1[1] = apply(tempData.y, threadScan1[2]);
            threadScan1[0] = s_out[*bi]
                           = apply(tempData.x, threadScan1[1]);
        }
        else
        {
            threadScan1[3] = identity();
            threadScan1[2] = apply(((i+2) < numElements) ? d_in[i+2] : identity(), threadScan1[3]);
            threadScan1[1] = apply(((i+1) < numElements) ? d_in[i+1] : identity(), threadScan1[2]);
            threadScan1[0] = s_out[*bi]
                           = apply((i     < numElements) ? d_in[i]   : identity(), threadScan1[1]);
        }
        __syncthreads();
    ...
    else
    {
        ...
        if (IS_FULLBLOCK || i + 3 < numElements)
        {
            ...
            threadScan0[1] = apply(tempData.y, threadScan0[0]);
            threadScan0[2] = apply(tempData.z, threadScan0[1]);
            threadScan0[3] = s_out[*ai]
                           = apply(tempData.w, threadScan0[2]);
        }
        else
        {
            threadScan0[0] = (i < numElements) ? d_in[i] : identity();
            threadScan0[1] = apply(((i+1) < numElements) ? d_in[i+1] : identity(), threadScan0[0]);
            threadScan0[2] = apply(((i+2) < numElements) ? d_in[i+2] : identity(), threadScan0[1]);
            threadScan0[3] = s_out[*ai]
                           = apply(((i+3) < numElements) ? d_in[i+3] : identity(), threadScan0[2]);
        }
        ...
        if (IS_FULLBLOCK || i + 3 < numElements)
        {
            ...
            threadScan1[1] = apply(tempData.y, threadScan1[0]);
            threadScan1[2] = apply(tempData.z, threadScan1[1]);
            threadScan1[3] = s_out[*bi]
                           = apply(tempData.w, threadScan1[2]);
        }
        else
        {
            threadScan1[0] = (i < numElements) ? d_in[i] : identity();
            threadScan1[1] = apply(((i+1) < numElements) ? d_in[i+1] : identity(), threadScan1[0]);
            threadScan1[2] = apply(((i+2) < numElements) ? d_in[i+2] : identity(), threadScan1[1]);
            threadScan1[3] = s_out[*bi]
                           = apply(((i+3) < numElements) ? d_in[i+3] : identity(), threadScan1[2]);
        }
        __syncthreads();
    }
}
                                       ...
                                       int biDev)
{
    ...
    if (IS_BACKWARD)
    {
        ...
        if (IS_EXCLUSIVE)
        {
            ...
            tempData.y = apply(temp, threadScan0[2]);
            tempData.x = apply(temp, threadScan0[1]);
        }
        else
        {
            tempData.w = apply(temp, threadScan0[3]);
            tempData.z = apply(temp, threadScan0[2]);
            tempData.y = apply(temp, threadScan0[1]);
            tempData.x = apply(temp, threadScan0[0]);
        }
        ...
        if (IS_EXCLUSIVE)
        {
            ...
            tempData.y = apply(temp, threadScan1[2]);
            tempData.x = apply(temp, threadScan1[1]);
        }
        else
        {
            tempData.w = apply(temp, threadScan1[3]);
            tempData.z = apply(temp, threadScan1[2]);
            tempData.y = apply(temp, threadScan1[1]);
            tempData.x = apply(temp, threadScan1[0]);
        }
    ...
    else
    {
        ...
        if (IS_EXCLUSIVE)
        {
            ...
            tempData.z = apply(temp, threadScan0[1]);
            tempData.w = apply(temp, threadScan0[2]);
        }
        else
        {
            tempData.x = apply(temp, threadScan0[0]);
        ...
        if (IS_EXCLUSIVE)
        {
            ...
            tempData.z = apply(temp, threadScan1[1]);
            tempData.w = apply(temp, threadScan1[2]);
        }
        else
        {
            tempData.x = apply(temp, threadScan1[0]);
...
__device__ void scanWarps(T x, T y,
                          T *s_data)
{
    T val  = warpscan(x, s_data);
    __syncthreads();
    T val2 = warpscan(y, s_data);

    int idx = threadIdx.x;

    if ((idx & 31)==31)
    {
        s_data[idx >> 5]                = apply(val, x);
        s_data[(idx + blockDim.x) >> 5] = apply(val2, y);
    }
    __syncthreads();
...
#endif
    {
        // maxlevel -> LOG_CTA_SIZE-LOG_WARP_SIZE+1
        s_data[idx] = warpscan(s_data[idx], s_data);
    }
    __syncthreads();

    val  = apply(val, s_data[idx >> 5]);

    val2 = apply(val2, s_data[(idx + blockDim.x) >> 5]);
