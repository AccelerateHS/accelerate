                                           ...
                                           int                baseIndex)
{
    ...
    int isLastBlock = (blockIdx.x == (gridDim.x-1));

    if (maxIndex < UINT_MAX)
    {
        // Since maxIndex is a 1 based index
        --maxIndex;
        int leftLess = address < maxIndex;
        int rightLess = (address + 7 * blockDim.x) < maxIndex;
            ...
            if (rightLess)
            {
                for (unsigned int i = 0; i < 8; ++i)
                    d_vector[address + i * blockDim.x] =
                        apply(d_vector[address + i * blockDim.x], uni[0]);
            }
            else
            {
                for (unsigned int i=0; i < 8; ++i)
                                                  ...
                                                  int                baseIndex)
{
    ...
    if (minIndex > 0)
    {
        ...
        --minIndex;
        int leftInRange = address > minIndex;
        int rightInRange = (address + 7 * blockDim.x) > minIndex;
