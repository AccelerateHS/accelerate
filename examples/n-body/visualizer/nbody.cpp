...
#ifdef __GLASGOW_HASKELL__
extern void __stginit_Visualize ( void );
#endif
...
class NBodyDemo
{
    public:
        ...
        static void init(int numBodies, int numDevices, int p, int q,
                         bool usePBO, bool useHostMem, bool useCpu, bool useCUDA)
        {
            m_singleton->_init(numBodies, numDevices, p, q, usePBO, useHostMem, useCpu, useCUDA);
        }
    ...
    private:
        ...
        void _init(int numBodies, int numDevices, int p, int q,
                   bool bUsePBO, bool useHostMem, bool useCpu, bool useCUDA)
        {
            if (useCpu)
            {
            ...
            else if (useCUDA)
            {
                ...
                m_nbodyAcc  = new BodySystemAcc(numBodies, p, q, bUsePBO, useHostMem);
                m_nbody = m_nbodyAcc;
                m_nbodyCpu = 0;
                m_nbodyCuda = 0;
            }
...
showHelp()
{
    ...
    printf("\t-cuda             (run n-body simulation on the CPU)\n");
    printf("\t-cpu              (run n-body simulation via CUDA)\n");
...
main(int argc, char **argv)
{
    ...
    useCUDA      = (checkCmdLineFlag(argc, (const char **) argv, "cuda") != 0);
    ...
    NBodyDemo<float>::init(numBodies, numDevsRequested, p, q, !(benchmark || compareToCPU || useHostMem), useHostMem, useCpu, useCUDA);
    NBodyDemo<float>::reset(numBodies, NBODY_CONFIG_SHELL);

    //Turning off double support as the accelerate code only supports floats. RCE: No reason this couldn't be implemented, just hasn't been done.
    // if (bSupportDouble)
    // {
    //     NBodyDemo<double>::Create();
    //     NBodyDemo<double>::init(numBodies, numDevsRequested, p, q, !(benchmark || compareToCPU || useHostMem), useHostMem, useCpu, useCUDA);
    //     NBodyDemo<double>::reset(numBodies, NBODY_CONFIG_SHELL);
    // }
