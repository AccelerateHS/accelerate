...
isLeft = sourceMap $ not . isRight

-- | Return 'True' if the argument is a 'Right'-value
--
isRight :: (HasCallStack, Elt a, Elt b) => Exp (Either a b) -> Exp Bool
isRight (Exp e) = sourceMap $ Exp $ SmartExp $ Pair mkAnn (SmartExp $ Prj mkAnn PairIdxLeft e) (SmartExp (Nil mkAnn))
...
fromLeft (Exp e) = sourceMap $ Exp $ SmartExp $ Prj mkAnn PairIdxRight $ SmartExp $ Prj mkAnn PairIdxLeft $ SmartExp $ Prj mkAnn PairIdxRight e
...
fromRight (Exp e) = sourceMap $ Exp $ SmartExp $ Prj mkAnn PairIdxRight $ SmartExp $ Prj mkAnn PairIdxRight e
...
either f g = sourceMap $ match \case
  Left_  x -> f x
  Right_ x -> g x
...
lefts es = sourceMap $ compact (map isLeft es) (map fromLeft es)
...
rights es = sourceMap $ compact (map isRight es) (map fromRight es)


instance Elt a => Functor (Either a) where
  fmap f = sourceMap $ either Left_ (Right_ . f)

instance Elt a => Monad (Either a) where
  return = sourceMap Right_
  x >>= f = sourceMap $ either Left_ f x

instance (Eq a, Eq b) => Eq (Either a b) where
  (==) = sourceMap $ match go
    where
      go :: SourceMapped => Exp (Either a b) -> Exp (Either a b) -> Exp Bool
...
instance (Ord a, Ord b) => Ord (Either a b) where
  compare = sourceMap $ match go
    where
      go :: SourceMapped => Exp (Either a b) -> Exp (Either a b) -> Exp Ordering
...
instance (Elt a, Elt b) => Semigroup (Exp (Either a b)) where
  ex <> ey = sourceMap (isLeft ex ? ( ey, ex ))
...
instance (Lift Exp a, Lift Exp b, Elt (Plain a), Elt (Plain b)) => Lift Exp (Either a b) where
  ...
  lift (Left a)  = sourceMap $ Left_ (lift a)
  lift (Right b) = sourceMap $ Right_ (lift b)
