...
module Data.Array.Accelerate.Sugar.Elt ( Elt(..), Tags(..) )
  where
...
--
-- These tags are intended to be equal to the tags generated by 'tagsR' from
-- 'Elt'. (This is currently not programmatically checked.) These should also
-- be equal to the tags for sum-type patterns generated using TemplateHaskell
-- (which is also not programmatically checked).
class Tags t where
  -- | The numeric 'TAG' values corresponding to the constructors of the type.
  -- The constructors are identified by their name as a String.
  tags :: [(String, TAG)]

  default tags :: GTags (Rep t) => [(String, TAG)]
  tags = gtags @(Rep t) 0

instance (Generic t, GTags (Rep t)) => Tags t
