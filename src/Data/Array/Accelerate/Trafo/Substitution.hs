...
isIdentityIndexing (Lam lhs (Body body))
  ...
  , Just Refl  <- bindingIsTrivial lhs vars = Just avar
...
rebuildPreOpenAcc k av acc =
  case acc of
    ...
    Stencil2 s1 s2 tp f b1 a1 b2 a2 -> Stencil2 s1 s2 tp <$> rebuildFun (pure . IE) av' f <*> rebuildBoundary av' b1 <*> k av a1 <*> rebuildBoundary av' b2 <*> k av a2
